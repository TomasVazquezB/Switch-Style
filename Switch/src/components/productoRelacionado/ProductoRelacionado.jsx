...
const ProductoRelacionado = ({ categoria, subCategoria }) => {
    ...
    return (
        <div className='my-24'>
            <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 gap-y-6'>{related.map((item, index) => (<ProductoItem key={index} id={item._id} img={item.img} nombre={item.nombre} precio={item.precio}/>))}</div>
        </div>
    )
}
