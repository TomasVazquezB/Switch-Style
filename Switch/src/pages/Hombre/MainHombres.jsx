    ...
    (async () => {
      try {
        const res = await axios.get('/ropa', {
          params: {genero: 'Hombre', estilo, categorias: subCategoria, tallas, orden: sortTipo === 'low-high' ? 'precio_asc' : sortTipo === 'high-low' ? 'precio_desc' : 'relevante',
          },
        });
    ...
    <div className="content">
        ...
        <div className="sidebar-content">
          ...
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-3 text-sm font-light text-gray-700 dark:text-gray-200">
              {ALL_SIZES.map((size) => (
                <button key={size} type="button" onClick={() => toggleTallaManual(size)} className={`px-4 py-2 border rounded-full text-sm transition-colors duration-200 ${tallas.includes(size) ? 'bg-black text-white border-black hover:bg-gray-800 dark:bg-white dark:text-black dark:border-white' : 'bg-white text-black border-gray-400 hover:bg-gray-100 dark:bg-gray-900 dark:text-white dark:border-gray-600'}`}>{size}</button>
              ))}
            </div>
          </div>
          ...
          <div className="mb-4">
            ...
            <div className="range flex items-center gap-2">
              <span>${precioMin}</span>
              <input type="range" min={precioMin} max={maxPrice} step="10" value={precioMax} onChange={(e) => setPrecioMax(Number(e.target.value))} className="w-full"/>
              <span>${precioMax}</span>
            </div>
          </div>
        </div>
      ...
      <div className="main pl-[220px] px-8 py-2">
        <div className="flex justify-end mb-3">
          <select value={sortTipo} onChange={(e) => setSortTipo(e.target.value)} className="border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm px-2 py-1 rounded">
        ...
        <div className="product-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtroProductos.map((item) => {
            const rawPath = item.imagen_url || item.ruta || item.ruta_imagen || item?.imagenes?.[0]?.ruta || '';
            const imageUrl = toRopaImageUrl(rawPath);
            const userId = getUserIdFromProduct(item);
            const uploader = userId == null ? null : (uploaderNames[userId] ?? `Usuario #${userId}`);

            return (
              <article key={item.id} className="rounded border overflow-hidden bg-white dark:bg-gray-800 transition">
                <ProductoItem id={item.id} img={imageUrl} nombre={item.titulo} precio={item.precio} tipo="ropa"/>
                <div className="px-4 pb-3">{uploader && (<p className="usuario-subido">Subido por: {uploader}</p>)}</div>
              </article>
            );
          })}

          {!filtroProductos.length && (
            <p className="col-span-full text-sm opacity-70 dark:text-gray-300">No encontramos resultados con los filtros seleccionados</p>
          )}
        </div>
      </div>
    </div>
