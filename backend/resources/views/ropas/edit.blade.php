...
<div class="max-w-3xl mx-auto mt-12 bg-white shadow-md rounded-lg p-8">
    ...
    <form id="edit-form" action="{{ route('ropas.update', $ropa->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="flex flex-wrap gap-4 items-center">
                @foreach ($tallas as $talla)
                    <div class="flex items-center gap-2">
                        ...
                        <input type="number" min="0" name="tallas[{{ $talla->id }}][cantidad]" value="{{ old('tallas.'.$talla->id.'.cantidad', $ropa->tallas->find($talla->id)?->pivot->cantidad ?? 0) }}"
                        ...
                        <div class="relative group border rounded overflow-hidden">
                            ...
                            <form action="{{ route('imagenes.destroy', $img->id) }}" method="POST"
                                ...
                                <button type="submit" onclick="return confirm('Eliminar esta imagen?')"
                                        class="bg-red-600 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs hover:bg-red-700 shadow">
                                    ✕
                                </button>
                            </form>

                            <!-- Botón principal o indicador -->
                            @if($ropa->ruta_imagen !== $img->ruta)
                                <form action="{{ route('imagenes.principal', $img->id) }}" method="POST"
                                      class="absolute bottom-1 left-1 z-10">
                                    @csrf
                                    @method('PUT')
                                    <button type="submit"
                                            class="bg-yellow-500 text-white text-xs px-2 py-1 rounded hover:bg-yellow-600 shadow">
                                        Principal
                                    </button>
                                </form>
                            @else
                                <span class="absolute bottom-1 left-1 bg-yellow-500 text-white text-xs px-2 py-1 rounded shadow">
                                    Principal
                                </span>
                            @endif
                        </div>
...
<script>
    ...
    document.querySelector('a[href="{{ route('ropas.index') }}"]').addEventListener('click', function (e) {
        if (isDirty && !confirm('Tienes cambios no guardados. ¿Seguro que quieres salir?')) {
            e.preventDefault();
        }
    });
</script>
